...
SQL;
        ...
        while ($item = $items->fetch(PDO::FETCH_ASSOC)) {
            ...
            Mage::helper('e2m')->writeInventoryFile($prefixPath, $product, $csvHeader, 'm2');
        }
    ...
    public function collectAttributesM2Action() {
        ...
        while ($attribute = fgetcsv($attributes, null, ',')) {
            ...
            $sourceModel = $isVariationSpecific ? Mage::getSingleton('core/resource')
                ->getConnection('core_read')->quote('Magento\\Eav\\Model\\Entity\\Attribute\\Source\\Table') : 'NULL';
            $type = $attribute['type'];
            $transactionSQL .= <<<SQL
    ...
    IF @attribute_id IS NULL THEN
        ...
        VALUES (@attribute_id, 1, 1, 0, 0, 1, 0, 1, 0, 0, 0, 0, 'simple,configurable', 1, 0, 0, 0);
    END IF;
...
SQL;
    ...
    IF NOT EXISTS(SELECT * FROM `eav_attribute_option_value` WHERE `option_id` = @option_id AND `store_id` = {$storeId} AND `value` = {$value}) THEN
